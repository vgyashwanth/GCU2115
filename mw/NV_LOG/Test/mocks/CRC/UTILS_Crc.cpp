...
//-------------------------------------------------------------------------------------------------/
static const uint16_t au16CRCTable[] = {
...
uint16_t Calculate_CRC16CCITXMODEM(const uint8_t *pu8Data, uint16_t u16Len)
{
    ...
    while (u16Len--)
    {
        ...
        u8Tmp = (uint8_t)(*pu8Data++ ^ u16CRC);
        u16CRC >>= 8;
        u16CRC ^= au16CRCTable[u8Tmp];
    }
    return u16CRC;
}
...
{
	uint16_t u16CRC = 0xFFFF;
	uint8_t* pu8Add = (uint8_t*) pu8Data;
	while(u32Len > 5000)
	{
		u16CRC = Calculate_CRCGeneric(pu8Add, 5000, u16CRC);
		pu8Add = pu8Add + 5000;
		u32Len = u32Len - 5000;
	}
	u16CRC = Calculate_CRCGeneric(pu8Add, (uint16_t) u32Len, u16CRC);
	return u16CRC;
}
